...
func TestFullSyncDelete(t *testing.T) {
	testDB := "test_sync_delete.db"
	os.Remove(testDB)
	st, err := store.NewStore(testDB)
	if err != nil {
		t.Fatal(err)
	...
	userID := "alice"
	// Запись с версией -1 (удалена)
	st.Put(userID, "to_delete", []byte("data"))
	st.PutVersion(userID, "to_delete", -1)

	// Фейковый сервер с каналом для проверки Delete
	lis := bufconn.Listen(1024 * 1024)
	srv := grpc.NewServer()
	fake := &fakeSyncServer{deleted: make(chan *syncpb.DeleteRequest, 1)}
	syncpb.RegisterSyncServer(srv, fake)
	go srv.Serve(lis)
	defer srv.Stop()
	...
	select {
	case req := <-fake.deleted:
		if req.EntryId != "to_delete" || req.Version != -1 {
